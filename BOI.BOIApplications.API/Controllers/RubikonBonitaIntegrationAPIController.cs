...
namespace BOI.BOIApplications.API.Controllers
{
    ...
    public class RubikonBonitaIntegrationAPIController : Controller
    {
        ...
        public async Task<ActionResult<PersonalCustomerInquiryResponse>> CorporateCustomerInquiry(string rcNumber)
        {
        ...
        public async Task<ActionResult<CustomerCreationResponse>> CreateCorporateAccount(CorporateCustomerAccountCreationRequest corporateAccountDetails)
            ...
            if (ModelState.IsValid)
            {
                ...
                return StatusCode(StatusCodes.Status404NotFound, new BaseResponse { Success = false, Message = "Unable to create account. Please check the detail and try again" });
            }
        ...
        public async Task<ActionResult<CustomerCreationResponse>> CreatePersonalAccount(PersonalCustomerAccountCreation personalAccountDetails)
        {
            if (ModelState.IsValid)
            {
        ...
        public async Task<ActionResult<CustomerCreationResponse>> LinkPersonalCustomerToCorporateCustomer(LinkPersonalCustomerToCorporateRequest accountLinkingDetails)
        {
            if (ModelState.IsValid)
            {
                var response = await _rubikonBonitaRepository.LinkCustomerAccount(accountLinkingDetails);
                if (response != null)
                {
                    return Ok(response);
                }
